...
// *** FUNCTION fGetNHood
// ***
// *** Get the 8-neighbourhood of voxel lLinInd
// ***
// ========================================================================
void fGetNHood(const long lLinInd, long& lNHoodSize, long* lNHood) {
	...
	if (lY > 0 && lX < lNX - 1)			lNHood[lNHoodSize++] = lLinInd - 1 + lNY;
	if (lX < lNX - 1)					lNHood[lNHoodSize++] = lLinInd + lNY;
	if (lX < lNX - 1 && lY < lNY - 1)	lNHood[lNHoodSize++] = lLinInd + 1 + lNY;

	if (lY < lNY - 1)					lNHood[lNHoodSize++] = lLinInd + 1;
	if (lY < lNY - 1 && lX > 0)			lNHood[lNHoodSize++] = lLinInd + 1 - lNY;
	if (lX > 0)							lNHood[lNHoodSize++] = lLinInd - lNY;
	if (lX > 0 && lX > 0 ) 				lNHood[lNHoodSize++] = lLinInd - 1 - lNY;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    if(nrhs < 4)  mexErrMsgTxt("At least 3 input arguments required.");
	...
	// 4th input: RG stoping difference.
	long lPixelNum = long(*mxGetPr(prhs[3]));
    ...
    bool *pbCandidate = new bool [lNX * lNY * lNZ]();
    ...
    // --------------------------------------------------------------------
	while ((lRegSize < lImSize) && (dLastDif < dMaxDif) && (lRegSize < lPixelNum)){
